...
Es un bloque de código SQL guardado dentro de la base de datos (**un objeto de la BD**) que  puede ejercutarse cuando se necesite
...
5. STORE PROCEDURES CON INSERTS , UPDATE Y DELETE
...
```sql
BEGIN TRY
    --CÓDIGO QUE PUED EGENERAR ERROR
END TRY
BEGIN CATCH
...
Obtener Información del error 'CATCH', SQL Server tiene funciones especiales:
...
>NOTA: ERRORES QUE NO CAPTURA EL TRY-CATCH
    - ERRORES DE COMPITACIÓN (EJ. Tabla que no existe )
    - Errores de Sintaxis
